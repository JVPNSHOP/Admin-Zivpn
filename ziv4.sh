...
# Status = only by Expire Date (not by UDP sessions)
...
# ===== password list build (safe, no array math) =====
...
def user_to_dict(row):
    ...
    # သက်တမ်းမကုန်သေးရင် => Online / Expiring
    # သက်တမ်းကုန်ပြီ => Offline
    if expire_at < now:
        status = "Offline"
    elif days_left <= 3:
        status = "Expiring"
...
def api_list_users():
    ...
    total = len(users)
    # Expire မကုန်သေး (Online + Expiring) = Online counter
...
# ---- index.html (နောက်ဆုံး UI version) ----
