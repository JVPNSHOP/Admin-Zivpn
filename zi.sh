#!/bin/bash
# ZIVPN UDP + Admin Panel One-Click Installer (with improved udp-session detection)
...
apt-get update -y
# upgrade ထုတ်ထားပြီး server reset မကျအောင်
# apt-get upgrade -y
...
# --- safe password list, no bash array tricks ---
...
# Termius မှာ စာမြင်ရအောင် echo on
...
# -------- ဗဟို UDP session counter (improved) ----------
def get_udp_sessions():
    """
    Try multiple methods to count active UDP flows on port 5667.
    Take the maximum value from:
      1) conntrack -L -p udp | grep 'dport=5667'
      2) /proc/net/nf_conntrack
      3) ss -u (sport or dport = 5667)
    """
    cmds = [
        "conntrack -L -p udp 2>/dev/null | grep 'dport=5667' | wc -l || echo 0",
        "grep -c ' dport=5667 ' /proc/net/nf_conntrack 2>/dev/null || echo 0",
        "ss -u '( sport = :5667 or dport = :5667 )' 2>/dev/null | tail -n +2 | wc -l || echo 0",
    ]
    best = 0
    for cmd in cmds:
        try:
...
def api_list_users():
    ...
    total = len(users)
    valid_users = [u for u in users if u["status"] != "Offline"]

    real_sessions = get_udp_sessions()

    if real_sessions > 0:
        online = min(real_sessions, len(valid_users))
    else:
        # detection fail / no flows => treat all as offline
        online = 0
...
# main index.html (unchanged from last version you liked)
    ...
    function renderUsers(data) {
      ...
      data.users.forEach(u => {
        ...
        card.innerHTML = `
          ...
          <div class="actions">
            <button class="btn btn-sm btn-ghost" type="button" onclick='editUser(${u.id},"${safeUser}","${safePass}","${u.expire_at}")'>✏ Edit</button>
...
echo -e "\e[32m Script Owner By: JueHtet \e[0m"
echo "========================================"
