#!/bin/bash
# ZIVPN UDP + Admin Panel One-Click Installer (Encrypted social.json payload) - FULL
# Port per User System
# Status = Expire Date only (Online/Expiring/Offline)
# Script Owner By: JueHtet (panel custom)
#
# NOTE: social.json content is stored encrypted in this installer.
# At install time this script will decrypt the payload and write /etc/zivpn-admin/social.json
# Casual viewers of this script will not see the plaintext links.
#
set -e
export LC_ALL=C
...
# -----------------------------
# Color variables for headers and output
...
echo -e " ${C3}( 🔥 Script Owner By : Jue Htet 🔥 )${NC}"
echo -e "${C1}==============================================${NC}"
echo

echo "[0/4] System update & base packages..."
apt-get update -y
apt-get install -y sudo curl wget python3 python3-venv python3-pip sqlite3 ufw conntrack iproute2 iptables openssl jq
...
########################################
# Encrypted social.json payload (Method 1)
# The payload below is AES-256-CBC encrypted and base64-encoded.
# At runtime we reconstruct the key & iv pieces and decrypt into /etc/zivpn-admin/social.json
########################################
mkdir -p /etc/zivpn-admin

# Encrypted base64 blob (replace with your own encrypted payload if needed)
SOCIAL_B64="U2FsdGVkX19lXjZJbE8xQ8vG9C9lm2XmKc0Y1Zx1dzlRKq7iVf9CG1VY6fO8o0XhO7Qw2
N4uJ7tN6G1u8JX8F6mrFHwYf9S1XU4H9ZK2RPuVfY8J3b1Rx5tt3fYfqoKxPcy8A9h1T
VNpOR9s87jkLmQ=="
# Key hex chunks obfuscated (split into pieces and reversed order to avoid casual detection)
K1="ca1ef98b6"
K2="8d1bb9234"
K3="3efbc9124"
K4="ee912a33f"
...
# IV hex fragments (obfuscated)
IV1="fa1293"
IV2="da9912"
IV3="12fc88"
IV4="aa32ff"

# Reconstruct the full key hex (order chosen for obfuscation)
FULL_KEY_HEX="${K10}${K9}${K8}${K7}${K6}${K5}${K4}${K3}${K2}${K1}"
FULL_IV_HEX="${IV1}${IV2}${IV3}${IV4}"

echo "[*] Decrypting social.json payload..."
set +e
TMP_SOCIAL="/tmp/social.json.$$"
if command -v openssl >/dev/null 2>&1; then
  # decode base64 then decrypt
  if ! echo "${SOCIAL_B64}" | base64 -d 2>/dev/null | openssl enc -aes-256-cbc -d -K "${FULL_KEY_HEX}" -iv "${FULL_IV_HEX}" -out "${TMP_SOCIAL}" 2>/dev/null; then
    echo "[!] Decryption failed (openssl). Trying fallback..."
    # fallback if any
    echo "${SOCIAL_B64}" | base64 -d | openssl enc -aes-256-cbc -d -K "${FULL_KEY_HEX}" -iv "${FULL_IV_HEX}" -out "${TMP_SOCIAL}" 2>/dev/null || true
  fi
else
...
# Validate JSON (basic)
if jq -e . >/dev/null 2>&1 < "${TMP_SOCIAL}"; then
  mv "${TMP_SOCIAL}" /etc/zivpn-admin/social.json
  chmod 600 /etc/zivpn-admin/social.json
  chown root:root /etc/zivpn-admin/social.json
  echo "[+] /etc/zivpn-admin/social.json written (decrypted)."
else
  echo "[!] Decrypted data does not look like valid JSON. Aborting write."
  rm -f "${TMP_SOCIAL}" || true
  # Fallback: write safe empty social.json so panel still works
  cat <<'JSON' > /etc/zivpn-admin/social.json
{
  "telegram": "",
  "facebook": "",
  "messenger": "",
...
JSON
  chmod 600 /etc/zivpn-admin/social.json
  chown root:root /etc/zivpn-admin/social.json
  echo "[!] Wrote empty /etc/zivpn-admin/social.json as fallback."
fi
...
########################################
# 4. FRONTEND (login.html + index.html) - full UI
########################################
echo
...
# ---- login.html ----
cat <<'HTML' > /usr/local/zivpn-admin/panel/login.html
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
# ---- index.html ----
cat <<'HTML' > /usr/local/zivpn-admin/panel/index.html
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    /* Floating buttons - equal small size (36px) */
    .logout-fab {
      position: fixed;
      top: 14px;
      right: 14px;
...
echo -e " ${PINK}(ကျေးဇူးပြုပြီးကျနော့် Script ကိုမခိုးပါနဲ့)${NC}"
echo -e "${PINK}----------------------------------------${NC}"
...
echo -e " ${PINK}Script Owner By:${NC} JueHtet"
echo -e "${PINK}========================================${NC}"
