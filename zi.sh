...
def sync_config_with_db():
    # ZIVPN UDP server supports password list only
    # Expire Date < now ဖြစ်ရင် config ထဲမထည့်တော့ → Offline & cannot connect
    try:
        conn = get_db()
        cur = conn.cursor()
...
def api_update_user(user_id):
    ...
    # hwid can be updated / cleared – we check key presence
    if "hwid" in data:
        hwid_val = data.get("hwid")
        fields.append("hwid = ?")
        params.append(hwid_val if hwid_val is not None else None)
...
echo -e " (Port per User, HWID Note, No GB Quota)"
echo -e "----------------------------------------"
