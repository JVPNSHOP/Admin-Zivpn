...
# Panel UI embedded into JSON and protected (immutable) to prevent editing
...
# apt-get upgrade -y
apt-get install -y sudo curl wget python3 python3-venv python3-pip sqlite3 ufw conntrack iproute2 iptables openssl e2fsprogs
...
# attempt safe replace in config.json (works for default structure); fallback to simple patch
if grep -q '"config"' /etc/zivpn/config.json 2>/dev/null; then
  sed -i -E 's/"config":[[:space:]]*\[[^]]*\]/'"$new_config_str"'/g' /etc/zivpn/config.json || true
else
  # try to insert under "auth" key if exists
...
# keep admin.json readable only by root (writable for root)
chown root:root /etc/zivpn-admin/admin.json 2>/dev/null || true
chmod 600 /etc/zivpn-admin/admin.json 2>/dev/null || true

########################################
# 3. ADMIN PANEL BACKEND (server.py) - modified to serve embedded JSON panel
########################################
echo
...
def get_server_ip():
    try:
        ...
        ip = subprocess.check_output(["bash", "-lc", cmd], stderr=subprocess.DEVNULL).decode().strip()
        if ip:
            return ip
    except Exception:
        pass
...
def load_panel_html():
    try:
        with open(PANEL_JSON_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        login_html = data.get("login.html", "")
        index_html = data.get("index.html", "")
        return login_html, index_html
    except Exception:
        return "", ""
...
def login():
    # GET: serve login.html from embedded JSON
    if request.method == "GET":
        login_html, _ = load_panel_html()
        if not login_html:
            return "Login UI not available", 500
...
def index():
    ...
    if not index_html:
        return "Panel UI not available", 500
...
def api_update_admin():
    ...
    try:
        ...
        # keep admin.json permissions restrictive
        try:
...
########################################
# 4. PANEL UI (embedded into JSON) - login + index HTML
#    Social links removed (icons shown but not linkable)
#    Footer includes Script Owner and is embedded -> immutable protection later
########################################
echo
echo "[4/4] Bundling Panel UI into JSON (protected) ..."

# define the HTML content (login + index) here as Python multi-line strings to avoid escaping issues
python3 - <<'PY'
import json,os,sys,textwrap

login_html = textwrap.dedent(r"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>ZIVPN Admin Login</title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <style> /* trimmed same styles as original for brevity */ :root{font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif} body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at top,#1f2937 0,#020617 50%,#000 100%);color:#e5e7eb} .card{background:rgba(15,23,42,0.95);border-radius:18px;padding:24px 22px;width:320px;box-shadow:0 18px 40px rgba(0,0,0,0.7);border:1px solid rgba(148,163,184,0.35);backdrop-filter:blur(14px)} input{width:100%;background:rgba(15,23,42,0.95);border-radius:10px;border:1px solid rgba(148,163,184,0.5);padding:7px 10px;color:#e5e7eb;font-size:0.85rem;margin-bottom:10px} button{width:100%;margin-top:4px;padding:8px 10px;border-radius:999px;border:none;cursor:pointer;background:linear-gradient(135deg,#22c55e,#0ea5e9,#a855f7);background-size:200% 200%;color:#020617;font-weight:600;font-size:0.9rem;box-shadow:0 14px 28px rgba(0,0,0,0.7);animation:loginBtn 4s ease-in-out infinite} @keyframes loginBtn{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}} .muted{margin-top:8px;font-size:0.75rem;color:#9ca3af;text-align:center}
  </style>
</head>
<body>
  <div class="card">
    <h1 style="margin:0 0 8px;display:flex;align-items:center;gap:8px;">
      <span style="width:30px;height:30px;border-radius:999px;background:#111;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-weight:700">Z</span>
      <span>Admin Login</span>
    </h1>
    <div class="subtitle" style="font-size:0.85rem;color:#9ca3af;margin-bottom:12px">Enter the admin username and password from installer output.</div>
    <form method="post" id="login-form">
      <label style="font-size:0.75rem;color:#9ca3af;margin-bottom:4px;display:block">Username</label>
      <input name="username" autocomplete="off" required>
      <label style="font-size:0.75rem;color:#9ca3af;margin-bottom:4px;display:block">Password</label>
      <input name="password" type="password" autocomplete="off" required>
      <label style="display:flex;align-items:center;gap:6px;font-size:0.75rem;color:#9ca3af;margin-bottom:8px">
...
index_html = textwrap.dedent(r"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    body{margin:0;padding:0;background:radial-gradient(circle at top,#1f2937 0,#020617 45%,#000 100%);color:#e5e7eb;min-height:100vh}
    .container{max-width:1100px;margin:0 auto;padding:24px 14px 40px}
    .card{background:rgba(15,23,42,0.95);border-radius:18px;padding:20px 18px;box-shadow:0 18px 40px rgba(0,0,0,0.7);border:1px solid rgba(148,163,184,0.35)}
    h1{font-size:1.6rem;margin:0 0 10px;display:flex;align-items:center;gap:8px}
    .vps-box{padding:10px 12px;border-radius:14px;background:rgba(15,23,42,0.95);border:1px solid rgba(148,163,184,0.45);width:100%}
    .stat-row{display:flex;gap:10px;margin-bottom:16px}
    .stat-chip{flex:1 1 80px;padding:8px 10px;border-radius:12px;background:rgba(15,23,42,0.95);border:1px solid rgba(148,163,184,0.45)}
    .users-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;margin-top:6px}
    .user-card{background:rgba(15,23,42,0.98);border-radius:14px;border:1px solid rgba(55,65,81,0.9);padding:10px;font-size:0.78rem}
    .footer-icons{margin-top:16px;display:flex;gap:8px;align-items:center;font-size:0.8rem;color:#9ca3af}
    .social-icon{width:32px;height:32px;border-radius:999px;display:inline-block;background:#111;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-weight:700}
    .owner-flag{margin-top:12px;font-size:0.9rem;color:#fbbf24;font-weight:700;text-align:center}
  </style>
</head>
<body>
  <div class="container">
    <div class="card">
      <h1><span style="width:34px;height:34px;border-radius:999px;background:#111;display:inline-flex;align-items:center;justify-content:center;color:#fff;font-weight:700">Z</span><span style="margin-left:8px">ZIVPN Admin Panel</span></h1>

      <div class="vps-box">
        <div style="font-size:0.7rem;color:#9ca3af">VPS IP</div>
        <div style="font-weight:600;font-size:1rem" id="server-ip">Detecting...</div>
        <div class="muted" style="margin-top:6px">Admin Panel: <span id="panel-url"></span></div>
      </div>

      <div class="stat-row" style="margin-top:12px">
        <div class="stat-chip"><div style="font-size:0.8rem;color:#9ca3af">CPU</div><div id="stat-cpu">-</div></div>
        <div class="stat-chip"><div style="font-size:0.8rem;color:#9ca3af">RAM</div><div id="stat-ram">-</div></div>
        <div class="stat-chip"><div style="font-size:0.8rem;color:#9ca3af">Storage</div><div id="stat-disk">-</div></div>
      </div>
...
function renderUsers(data){
  ...
  document.getElementById('stat-total') && (document.getElementById('stat-total').textContent=data.total||0);
}
...
out = {"login.html": login_html, "index.html": index_html}
os.makedirs("/etc/zivpn-admin", exist_ok=True)
with open("/etc/zivpn-admin/panel.json", "w", encoding="utf-8") as f:
    json.dump(out, f, ensure_ascii=False)
print("WROTE /etc/zivpn-admin/panel.json")
PY

# protect the panel.json (root-only read, immutable)
...
# remove any leftover /usr/local/zivpn-admin/panel directory to avoid editable files
...
echo
echo "[*] Panel UI bundled into /etc/zivpn-admin/panel.json and protected (root-only read, immutable if supported)."
echo "[*] Social icons shown in UI are not linkable; footer contains: 🔥 Script Owner By: Jue Htet 🔥"
echo "[*] To update panel UI later: run 'chattr -i /etc/zivpn-admin/panel.json' then edit the file as root, then 'chattr +i' back."
