...
# Quota chain for per-user ports
...
def get_db():
    # DB connection ကို multi-thread safe + timeout နဲ့ထားပြီး lock error နည်းအောင်
...
def api_create_user():
    ...
    try:
        ...
        # DB error (locked / others) ကို JSON အဖြစ်ပြန်ပေး
        conn.close()
...
# ---- index.html (Panel with Port + Flow + Used/Left + CPU/RAM/DISK + Settings) ----
  ...
  <script>
    ...
    async function createUser(ev) {
      ...
      try {
        ...
        // login session ပြတ်သွားရင် /login redirect လို့ JSON မရတော့တာကို handle
        if (res.status === 401 || res.redirected) {
