...
def init_db():
    ...
    # Add quota_gb column if missing
...
def user_to_dict(row):
    ...
    # Expire Date သာကြည့်ပြီး Status သတ်မှတ်
    if expire_at < now:
        status = "Offline"
    elif days_left <= 3:
        status = "Expiring"
...
def api_create_user():
    ...
    try:
        cur.execute(
            "INSERT INTO users (username, password, created_at, expire_at, quota_gb) "
            "VALUES (?, ?, ?, ?, ?)",
            (username, password, created_at, expire_at, quota_gb),
        )
        conn.commit()
...
# ---- Admin username/password change API ----
...
# ---- index.html (Admin Panel + Settings + Flow/Stats) ----
