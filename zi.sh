#!/bin/bash
# ZIVPN UDP + Admin Panel One-Click Installer (with conntrack online count)
...
apt-get update -y
# upgrade လုပ်ရင် kernel / network restart ဖြစ်နိုင်လို့ ဖျောက်ထားသည်
# apt-get upgrade -y
apt-get install -y sudo curl wget python3 python3-venv python3-pip sqlite3 ufw conntrack
...
# --- safe password list, no arrays (bash 4/5 compatible) ---
...
# show password while typing so Termius keyboard can see
...
# ---- conntrack based UDP session counter (real active sessions) ----
def get_udp_sessions():
    """
    Count active UDP connections that hit port 5667 (ZIVPN UDP).
    We grep 'dport=5667' to ignore conntrack header line, etc.
    """
    try:
        cmd = "conntrack -L -p udp 2>/dev/null | grep 'dport=5667' | wc -l"
        out = subprocess.check_output(["bash", "-lc", cmd]).decode().strip()
        return int(out) if out else 0
    except Exception:
        return 0

# Online logic per-user (expire based)
...
def api_list_users():
    ...
    real_sessions = get_udp_sessions()  # conntrack count
