...
# Quota = Per-user Port + iptables Counters
...
# password list build (array math မသုံးပဲ safe string)
...
# Quota chain for per-user ports (use raw/PREROUTING so DNAT မလုပ်ခင် bytes ကို တွက်ပေးထားဖို့)
...
QUOTA_TABLE = "raw"  # per-user quota ကို DNAT မလုပ်ခင် PREROUTING မှာတွက်ဖို့ raw table သုံး
...
def init_db():
    conn = get_db()
    cur = conn.cursor()
    # create full schema (new installs)
    cur.execute(
        """
    ...
    # check existing columns (for old DB)
    cur.execute("PRAGMA table_info(users)")
    cols = [r[1] for r in cur.fetchall()]
    need_cols = {"id", "username", "password", "created_at", "expire_at", "quota_gb", "udp_port", "used_bytes"}
    ...
    if missing:
        ...
        if count == 0:
            # no data → drop & recreate clean table
...
def ensure_quota_chain():
    try:
        subprocess.run(
            ["bash", "-lc",
             f"iptables -t {QUOTA_TABLE} -N {QUOTA_CHAIN} 2>/dev/null || true; "
             f"iptables -t {QUOTA_TABLE} -C PREROUTING -p udp --dport 6000:19999 -j {QUOTA_CHAIN} 2>/dev/null || "
             f"iptables -t {QUOTA_TABLE} -A PREROUTING -p udp --dport 6000:19999 -j {QUOTA_CHAIN}"],
            check=False
        )
    except Exception as e:
...
def ensure_quota_rule(port: int):
    try:
        ensure_quota_chain()
        cmd = f"iptables -t {QUOTA_TABLE} -C {QUOTA_CHAIN} -p udp --dport {port} -j RETURN 2>/dev/null"
        r = subprocess.run(["bash", "-lc", cmd], check=False)
        if r.returncode != 0:
            add_cmd = f"iptables -t {QUOTA_TABLE} -A {QUOTA_CHAIN} -p udp --dport {port} -j RETURN"
...
def remove_quota_rule(port: int):
    try:
        cmd = f"iptables -t {QUOTA_TABLE} -D {QUOTA_CHAIN} -p udp --dport {port} -j RETURN 2>/dev/null"
...
def read_quota_counters():
    ...
    try:
        ...
        out = subprocess.check_output(
            ["bash", "-lc", f"iptables -t {QUOTA_TABLE} -nvx -L {QUOTA_CHAIN} 2>/dev/null || true"]
        ).decode()
...
def enforce_quota_once():
    # Quota ကိုအလိုအလျောက် expire မလုပ်တော့
    # used_bytes ကို iptables counter နဲ့ update ပဲလုပ်ပေးမယ်
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT id, udp_port, quota_gb, used_bytes FROM users")
        rows = cur.fetchall()
    except sqlite3.OperationalError as e:
        print("enforce_quota_once db error:", e)
        conn.close()
        return
    ...
    for r in rows:
        uid = r["id"]
        port = r["udp_port"]
        quota = r["quota_gb"]  # UI က left_gb တွက်ဖို့အတွက်သိထားရုံ
        used_old = r["used_bytes"] if r["used_bytes"] is not None else 0

        if port is None:
            continue

        raw_bytes = counters.get(port)

        if raw_bytes is None:
            used_bytes_now = used_old
        elif raw_bytes >= used_old:
            used_bytes_now = raw_bytes
        else:
            # iptables counter reset ဖြစ်လိုက်ရင် fallback
            used_bytes_now = used_old + raw_bytes
