#!/bin/bash
# ZIVPN UDP + Admin Panel One-Click Installer (with hybrid udp-session detection)
...
apt-get update -y
# Server reset ဖြစ်နိုင်လို့ upgrade မလုပ်တော့
# apt-get upgrade -y
...
# Termius မှာ မြင်ရစေဖို့ echo on
...
# ---- HYBRID UDP session counter ----
def get_udp_sessions():
    """
    Try multiple methods to count active UDP connections to port 5667.
    - conntrack -L (BEST, real NAT sessions)
    - ss -u state established dport=:5667 (fallback)
    Return the maximum count from both.
    """
    cmds = [
        "conntrack -L -p udp 2>/dev/null | grep 'dport=5667' | wc -l",
        "ss -u state established '( dport = :5667 )' 2>/dev/null | tail -n +2 | wc -l"
    ]
    best = 0
    for cmd in cmds:
        try:
...
# main panel (UI) – same as last version (with copy, social icons, theme, toast)
