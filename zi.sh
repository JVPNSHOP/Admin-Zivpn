...
if [ -n "$DEV" ]; then
  # ports 6000-19999 ကို UDP main port 5667 ထဲ DNAT
...
# Quota chain for per-user ports (raw/PREROUTING → DNAT မလုပ်ခင် bytes တွက်ဖို့)
...
QUOTA_TABLE = "raw"  # DNAT မလုပ်ခင် PREROUTING မှာ traffic တွက်ဖို့ raw table သုံး
...
def init_db():
    ...
    need_cols = {"username", "password", "created_at", "expire_at", "quota_gb", "udp_port", "used_bytes"}
...
def sync_config_with_db():
    try:
        ...
        # original config.json သုံးသလို top-level "config" ကိုပဲ update မှာ
        data["config"] = passwords if passwords else ["zi"]
...
<head>
  ...
  <!-- (login UI သည် မင်းပေးထားသလိုအပြောင်းအလဲမလုပ်) -->
  <style>
...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <!-- မင်းပေးထားခဲ့တဲ့ panel design ကို အတိုင်းပဲ ထားထား -->
  <style>
    :root {
