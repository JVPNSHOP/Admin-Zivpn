#!/bin/bash
# Full ZIVPN UDP + Admin Panel One-Click Installer
# Embeds provided login.html + index.html into /etc/zivpn-admin/panel.json (immutable)
...
apt-get update -y
# skip full upgrade to avoid unexpected reboots
apt-get install -y sudo curl wget python3 python3-venv python3-pip sqlite3 ufw conntrack iproute2 iptables openssl e2fsprogs -y
...
if [ -z "$input_config" ]; then
  ...
  for p in "${arr[@]}"; do
    p_trim="$(echo "$p" | xargs)"
    [ -z "$p_trim" ] && continue
    # escape quotes not strictly necessary here, we'll write into JSON via python later for safety
...
# Try to patch config.json safely via python to avoid sed regex issues
python3 - <<PY || true
import json,sys
...
try:
    ...
    d={}
# place passwords under top-level "auth"->"config" to match server expectation
d.setdefault("auth",{})
...
def sync_config_with_db():
    try:
        ...
        cur.execute("SELECT password, expire_at FROM users"); rows=cur.fetchall(); conn.close()
...
def api_update_user(user_id):
    ...
    conn=get_db(); cur=conn.cursor(); cur.execute(f"UPDATE users SET {', '.join(fields)} WHERE id = ?", params); conn.commit(); conn.close()
    sync_config_with_db()
    return jsonify({"success":True})
...
########################################
# 4. PANEL UI: write the exact login.html and index.html provided by user,
#    then bundle them into /etc/zivpn-admin/panel.json and protect it.
########################################
echo
echo "[4/4] Writing provided panel HTML files and bundling into JSON..."

# write login.html (use the HTML you provided earlier)
...
# write index.html (use the HTML you provided earlier)
